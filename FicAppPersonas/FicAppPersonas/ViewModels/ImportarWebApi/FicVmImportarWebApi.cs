...
namespace FicAppPersonas.ViewModels.ImportarWebApi
{
    public class FicVmImportarWebApi : INotifyPropertyChanged
    {
        ...
        public FicVmImportarWebApi(IFicSrvImportarWebApi IFicSrvImportarWebApi)
        {
        ...
        public ICommand FicMecImportIdInv
        {
            get
            {
                return _FicMecImportIdInv = _FicMecImportIdInv ??
                      new FicVmDelegateCommand(FicMecImportPersonaId);
            }
        }//ESTE VENTO AGREGA EL COMANDO AL BOTON EN LA VIEW

        private async void FicMecImportPersonaId()
        {
            try
            {
                if (_FicLabelIdInv.Length > 0)
                {
                    _FicTextAreaImpInv = await IFicSrvImportarWebApi.FicGetImportPersonas(int.Parse(_FicLabelIdInv));
        ...
        public ICommand FicMecImportInv
        {
            get
            {
                return _FicMecImportInv = _FicMecImportInv ??
                      new FicVmDelegateCommand(FicMecImportPersonas);
            }
        }//ESTE VENTO AGREGA EL COMANDO AL BOTON EN LA VIEW

        private async void FicMecImportPersonas()
        {
            try
            {
                _FicTextAreaImpInv = await IFicSrvImportarWebApi.FicGetImportPersonas(0);
                RaisePropertyChanged("FicTextAreaImpInv");
                await new Page().DisplayAlert("ALERTA", "Datos Actualizados.", "OK");
            }
            catch (Exception e)
